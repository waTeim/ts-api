...
function genSwaggerPaths(def, controllers) {
    ...
    for (var i = 0; i < controllers.length; i++) {
        ...
        for (var j = 0; j < methods.length; j++) {
            ...
            var inputForm = "query";
            var responses = {};
            var path_1 = { tags: [p1], operationId: p2, produces: ["application/json"], parameters: parameters, responses: responses };
            ...
            for (var k = 0; k < methods[j].methodParameters.length; k++) {
                ...
                var parameterTypeDef = typeToJSON(parameter.type, null);
                parameters.push({
                    name: parameter.id,
                    in: inputForm,
                    required: true,
