...
function genSwaggerPaths(def, controllers) {
    ...
    for (var i = 0; i < controllers.length; i++) {
        ...
        var comment = symtab[controllers[i].name];
        for (var j = 0; j < methods.length; j++) {
            ...
            var inputForm = "query";
            var responses = {};
            var path_1 = { tags: [p1], operationId: p2, parameters: parameters, responses: responses };
...
function genSources(items, packageName, checkFile, swaggerFile) {
    ...
    var contents = "const Ajv = require('ajv');";
...
function generate(patterns, options, packageName, checkFile, swaggarFile) {
    ...
    function visit(node) {
        if (node.decorators != null) {
            ...
            var comment = void 0;
            if (name_4 != null)
                symbol = checker.getSymbolAtLocation(name_4);
            if (name_4 != null)
