    ...
    for (var i = 0; i < controllers.length; i++) {
        ...
        for (var j = 0; j < methods.length; j++) {
            ...
            if (methods[j].returnType != null) {
                ...
                console.log("void return from a function", p2);
...
function genRoutes(endpoints, routers, controllers, srcRoot, routesFile) {
    ...
    for (var i = 0; i < endpoints.length; i++) {
        ...
        output += "\n";
        output += "  apex.getRouter('" + endpoints[i].className + "')." + rfunc + "('/" + path_4 + "', async(req,res,next) => {\n";
        output += "    try {\n";
    ...
    for (var i = 0; i < controllers.length; i++) {
        var path_5 = controllers[i].path;
        if (path_5.charAt(0) == '/')
            path_5 = path_5.substring(1);
        if (path_5.charAt(path_5.length - 1) == '.')
            path_5 = path_5.substring(0, path_5.length - 1);
        output += "  apex.app.use(apex.prefix + '/" + path_5 + "',apex.getRouter('" + controllers[i].className + "'));\n";
    }
