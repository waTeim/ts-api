...
function typeliteralToJSON(typeDesc, jsDoc, options) {
    var typeliteralDesc = typeDesc;
    var res = { oneOf: [] };
    for (var i = 0; i < typeliteralDesc.members.length; i++) {
        var typeliteralMember = typeToJSON(typeliteralDesc.members[i], null, options);
        if (typeliteralMember != null)
            res.oneOf.push(typeliteralMember);
    }
    return res;
}
function tupleTypeToJSON(typeDesc, jsDoc, options) {
    var tupleDesc = typeDesc;
    var res = { allOf: [] };
    for (var i = 0; i < tupleDesc.elementTypes.length; i++) {
        var tupleElement = typeToJSON(tupleDesc.elementTypes[i], null, options);
        if (tupleElement != null)
            res.allOf.push(tupleElement);
    }
    return res;
}
function indexedAccessTypeToJSON(typeDesc, jsDoc, options) {
    var iaDesc = typeDesc;
    var indexType = checker.getTypeFromTypeNode(typeDesc.indexType);
    var index = checker.typeToString(indexType);
    var obj = typeToJSON(typeDesc.objectType, jsDoc, { expandRefs: true });
    var res;
    ...
    if (typeDesc.constructor.name == 'NodeObject') {
        ...
        switch (typeDesc.kind) {
            ...
            case ts.SyntaxKind.TypeLiteral:
                res = typeliteralToJSON(typeDesc, jsDoc);
                break;
            case ts.SyntaxKind.PropertySignature:
                {
                    var propertySignatureDesc = typeDesc;
                    if (propertySignatureDesc.type != null)
                        res = typeToJSON(propertySignatureDesc.type, jsDoc);
                    else
                        res = null;
                }
                break;
            case ts.SyntaxKind.TupleType:
                res = tupleTypeToJSON(typeDesc, jsDoc);
                break;
            case ts.SyntaxKind.IndexedAccessType:
                res = indexedAccessTypeToJSON(typeDesc, jsDoc);
                break;
