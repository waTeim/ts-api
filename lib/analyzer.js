 ...
 * @param {Controller[]} array of controller definitions.
 */
function genSwaggerTags(def, router, controllers) {
    var tags = [];
    for (var i = 0; i < controllers.length; i++) {
...
function genSwaggerRequestBody(router, controller, method, methodPathDecomposition) {
    var parameters = [];
    for (var i = 0; i < method.methodParameters.length; i++) {
        var parameter = method.methodParameters[i];
 ...
 * @param {Controller[]} array of controller definitions.
 */
function genSwaggerPaths(def, router, controllers) {
    var paths = {};
    ...
    for (var i = 0; i < controllers.length; i++) {
        ...
        for (var j = 0; j < methods.length; j++) {
            ...
            if (methodType == "post" || methodType == "all" || methodType == "put" || methodType == "patch") {
                ...
                path_3.requestBody = genSwaggerRequestBody(router, controllers[i], methods[j], methodPathDecomposition);
            }
            else
 ...
 * @param {Controller[]} array of controller definitions.
 */
function genSwaggerRoutes(def, router, controllers) {
...
function genExpressRoutes(endpoints, router, controllers, srcRoot, routesFile) {
    ...
    for (var i = 0; i < endpoints.length; i++) {
        ...
        if (endpoints[i].decoratorArgs.length != 0)
            path_4 = endpoints[i].decoratorArgs[0];
        var endpointPathDecomposition = decomposePath(path_4);
                ...
                for (var decoratorName in parm.decorators) {
                    if (decoratorName == 'urlParam') {
                        ...
                        if (decoratorArgs.length)
                            output += "req.params." + decoratorArgs[0] + ",";
                        else
    ...
    for (var i = 0; i < controllers.length; i++) {
        var path_5 = '/' + decompositionToPath(controllers[i].decomposition, "express");
        if (routerPath != "")
            path_5 = "/" + routerPath + path_5;
        output += "  apex.app.use('" + path_5 + "',apex.getRouter('" + controllers[i].className + "'));\n";
    }
    var docPath = '/docs';
    if (routerPath != "")
