...
function genExpressRoutes(endpoints, router, controllers, srcRoot, routesFile) {
    ...
    for (var i = 0; i < endpoints.length; i++) {
                ...
                for (var decoratorName in parm.decorators) {
                    if (decoratorName == 'urlParam') {
                        ...
                        if (decoratorArgs.length)
                            params.push({ id: decoratorArgs[0], type: "urlParam" });
                        else
        ...
        for (var j = 0; j < params.length; j++) {
            if (j != 0)
                output += ',';
            if (params[j].type == "urlParam")
                output += "req.params." + params[j].id;
            else {
                if (params.length - numURLParam == 1) {
                    ...
                    if (rfunc == 'get')
                        output += "req.query." + params[j].id;
                    else
                        output += "req.body." + params[j].id;
                }
            }
        }
        output += ");\n\n";
