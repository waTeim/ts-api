...
function genRoutes(endpoints, routers, controllers, srcRoot, routesFile) {
    ...
    for (var i = 0; i < routers.length; i++) {
        var fileName = path.resolve(routers[i].fileName);
        if (srcRoot != null) {
            fileName = fileName.replace(srcRoot + '/', '');
            fileName = fileName.replace(path.extname(fileName), "");
            ...
            output += "const " + routers[i].className + "= require('./" + path.basename(fileName) + "');\n";
    }
    ...
    for (var i = 0; i < routers.length; i++) {
        ...
        for (var j = 0; j < endpoints.length; j++) {
            ...
            output += "\n";
            output += "  pod.getRouter('" + endpoints[j].className + "')." + rfunc + "('/" + path_4 + "', async(req,res,next) => {\n";
            output += "    try {\n";
        ...
        for (var j = 0; j < controllers.length; j++) {
            ...
            if (path_5.charAt(prefix.length - 1) == '.')
                path_5 = prefix.substring(0, path_5.length - 1);
            output += "  app.use('/" + prefix + "/" + path_5 + "',pod.getRouter('" + controllers[j].className + "'));\n";
        }
    }
    output += "  return pods;\n";
    output += "}\n";
